...
def base_testing(config, device, base_output_folder, model_file_path, model, wind_angles, X_test_tensor, feature_scaler, target_scaler, plot_folder, overall_start_time, testing_type, y_test_tensor=None, data_folder=None):
    ...
    vtk_output_folder = os.path.join(base_output_folder, f'vtk_output')
    ...
    if y_test_tensor is not None:
        ...
        df = evaluate_model(config, model, wind_angles, X_test_tensor, feature_scaler, target_scaler, None, data_folder, testing_type, vtk_output_folder=None)
        print (f'Model Evaluated and Starting to Plot, Time: {(time.time() - overall_start_time):.2f} Seconds')
        if config["plotting"]["make_plots"]:
            plot_data_2d(config,df,wind_angles,geometry_filename,plot_folder,single=True)
        print (f'Plotting Done, Time: {(time.time() - overall_start_time):.2f} Seconds')

def base_testing_PCA(config, device, base_output_folder, model_file_path, model, wind_angles, plot_folder, overall_start_time, testing_type, data_folder, X, y, x):
    chosen_machine_key = config["chosen_machine"]
    datafolder_path = os.path.join(config["machine"][chosen_machine_key], config["data"]["data_folder_name"])
    geometry_filename = os.path.join(datafolder_path, config["data"]["geometry"])
    if config["testing"]["epoch_number"] is not None:
    ...
    vtk_output_folder = os.path.join(base_output_folder, f'vtk_output')
    os.makedirs(vtk_output_folder, exist_ok=True)
    df = evaluate_model_PCA(config, model, wind_angles, data_folder, testing_type, vtk_output_folder, X, y, x)
    print (f'Model Evaluated and Starting to Plot, Time: {(time.time() - overall_start_time):.2f} Seconds')
    if config["plotting"]["make_plots"]:
        plot_data_2d(config,df,wind_angles,geometry_filename,plot_folder,single=False)
        plot_diff_2d(config,df,wind_angles,geometry_filename,plot_folder)
...
def base_testing_fft(config, device, base_output_folder, model_file_path, model, wind_angles, plot_folder, overall_start_time, testing_type, data_folder, X, y, x):
    chosen_machine_key = config["chosen_machine"]
    datafolder_path = os.path.join(config["machine"][chosen_machine_key], config["data"]["data_folder_name"])
    geometry_filename = os.path.join(datafolder_path, config["data"]["geometry"])
    if config["testing"]["epoch_number"] is not None:
    ...
    vtk_output_folder = os.path.join(base_output_folder, f'vtk_output')
    os.makedirs(vtk_output_folder, exist_ok=True)
    df = evaluate_model_fft(config, model, wind_angles, data_folder, testing_type, vtk_output_folder, X, y, x)
    print (f'Model Evaluated and Starting to Plot, Time: {(time.time() - overall_start_time):.2f} Seconds')
    if config["plotting"]["make_plots"]:
        plot_data_2d(config,df,wind_angles,geometry_filename,plot_folder,single=False)
        plot_diff_2d(config,df,wind_angles,geometry_filename,plot_folder)
...
def testing(model, device, config, data_dict, model_file_path, output_folder, today, overall_start_time):
    ...
    print ('training wind angles at the source - ', training_wind_angles)
    if config["training"]["use_PCA"]:
        X_train_tensor, y_train_tensor, X_test_tensor_skipped, y_test_tensor_skipped, x_training, x_skipped = data_dict.values()
        base_testing_PCA(config, device, output_folder, model_file_path, model, training_wind_angles, testing_plot_folder, overall_start_time, testing_type, testing_data_folder, X_train_tensor, y_train_tensor, x_training)
    elif config["training"]["use_fft"]:
        X_train_tensor, y_train_tensor, X_test_tensor_skipped, y_test_tensor_skipped, x_training, x_skipped = data_dict.values()
        base_testing_fft(config, device, output_folder, model_file_path, model, training_wind_angles, testing_plot_folder, overall_start_time, testing_type, testing_data_folder, X_train_tensor, y_train_tensor, x_training)
    else:
...
def evaluation(model, device, config, data_dict, model_file_path, output_folder, today, overall_start_time):
    ...
    if config["training"]["use_PCA"]:
        X_train_tensor, y_train_tensor, X_test_tensor_skipped, y_test_tensor_skipped, x_training, x_skipped = data_dict.values()
        base_testing_PCA(config, device, output_folder, model_file_path, model, skipped_wind_angles, skipped_plot_folder, overall_start_time, testing_type, skipped_data_folder, X_test_tensor_skipped, y_test_tensor_skipped, x_skipped)
    elif config["training"]["use_fft"]:
        X_train_tensor, y_train_tensor, X_test_tensor_skipped, y_test_tensor_skipped, x_training, x_skipped = data_dict.values()
        base_testing_fft(config, device, output_folder, model_file_path, model, skipped_wind_angles, skipped_plot_folder, overall_start_time, testing_type, skipped_data_folder, X_test_tensor_skipped, y_test_tensor_skipped, x_skipped)
    else:
        X_train_tensor, y_train_tensor, X_test_tensor, y_test_tensor, labels_train_tensor, X_train_tensor_skipped, y_train_tensor_skipped, X_test_tensor_skipped, y_test_tensor_skipped, feature_scaler, target_scaler, X_train_divergences_tensor, X_test_divergences_tensor, y_train_divergences_tensor, y_test_divergences_tensor = data_dict.values()
