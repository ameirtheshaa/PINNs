...
def make_double_plots(figsize, z_actual, z_pred, grid1, grid2, grid3_actual, grid3_pred, geometry, quiver_points1, quiver_points2, quiver_v1_actual, quiver_v2_actual, quiver_v1_pred, quiver_v2_pred, plane, wind_angle, cut, tolerance, cbar_label, savename, arrow10 = None, arrow20 = None, arrow11 = None, arrow21 = None):
    ...
    for triangle in geometry:
        ...
        if plane == 'X-Z':
            x, z = normalize_grids(plane, x, z)
            axs[0].plot(x, z, color='black')
        if plane == 'Y-Z':
            y, z = normalize_grids(plane, y, z)
            axs[0].plot(y, z, color='black')
...
def plot_data(df, plane, wind_angle, cut, tolerance, geometry_filename, savenames):
    ...
    for grid3 in all_grids:
        make_singular_plot((16, 8), filtered_df[savenames[counter][0]].values, grid1, grid2, grid3, geometry, quiver_points1, quiver_points2, quiver_v1, quiver_v2, plane, wind_angle, cut, tolerance, savenames[counter][1], savenames[counter][2])
...
def plot_data_predictions(df, plane, wind_angle, cut, tolerance, geometry_filename, savenames):
    ...
    for grid3_actual, grid3_pred in all_grids:
        make_double_plots((16, 8), filtered_df[savenames[counter][0]].values, filtered_df[savenames[counter][0].replace("_Actual", "_Predicted")].values, grid1, grid2, grid3_actual, grid3_pred, geometry, quiver_points1, quiver_points2, quiver_v1_actual, quiver_v2_actual, quiver_v1_pred, quiver_v2_pred, plane, wind_angle, cut, tolerance, savenames[counter][1], savenames[counter][2], arrow10 = None, arrow20 = None, arrow11 = None, arrow21 = None)
        # if counter == 0 and plane == 'X-Y':
...
def plot_data_2d(df,wind_angles,geometry_filename,plot_folder,single):
    params = [['X-Z',-300,5],['Y-Z',-300,5],['X-Y',5,5]]
