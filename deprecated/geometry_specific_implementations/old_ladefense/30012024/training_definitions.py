...
def calculate_total_loss(X, y, epoch, config, model, optimizer, closure, no_slip_features_normals=None, all_inlet_features_targets=None):
    ...
    if config["loss_components"]["momentum_loss"]:
        loss_components['momentum_loss'] = model.compute_physics_momentum_loss(model, X, input_params, output_params, rho, nu)
