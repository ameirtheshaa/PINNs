...
def main(base_directory, config, output_zip_file=None):
    ...
    model = PINN(input_params=config["training"]["input_params"],
    ...
    if config["train_test"]["test"]:
        ...
        if config['TPU']:
            checkpoint = tf.keras.models.load_model(model_file_path, map_location='cpu')
            model.load_state_dict(checkpoint)
            model.to(device)
        else:
            checkpoint = tf.keras.models.load_model(model_file_path, map_location=device)
