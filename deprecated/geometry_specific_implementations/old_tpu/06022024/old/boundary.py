...
def parameters(datafolder_path, config):
    ...
    return num_points_sphere, num_points_cylinder, epsilon, x_range, y_range, faces_sphere, faces_cylinder, num_points_boundary
...
def sample_domain_boundary(config, device, filenames, datafolder_path, x_range, y_range, wind_direction, num_points, feature_scaler, target_scaler):
    df_all = []
    for filename in sorted(filenames):
        wind_angle = int(filename.split('_')[1].split('dd')[1])
        if wind_direction == wind_angle:
            ...
            for i in df.values:
                ...
                # Side 1
                side_1 = [(i / (num_points - 1) * 1000, 0, altitude, np.cos(np.deg2rad(wind_angle)), np.sin(np.deg2rad(wind_angle)),  u_value, v_value, w_value) for i in range(num_points)]
                # Side 2
                side_2 = [(1000, i / (num_points - 1) * 1000, altitude, np.cos(np.deg2rad(wind_angle)), np.sin(np.deg2rad(wind_angle)), u_value, v_value, w_value) for i in range(num_points)]
                # Side 3
                side_3 = [(1000 - i / (num_points - 1) * 1000, 1000, altitude, np.cos(np.deg2rad(wind_angle)), np.sin(np.deg2rad(wind_angle)), u_value, v_value, w_value) for i in range(num_points)]
                # Side 4
                side_4 = [(0, 1000 - i / (num_points - 1) * 1000, altitude, np.cos(np.deg2rad(wind_angle)), np.sin(np.deg2rad(wind_angle)), u_value, v_value, w_value) for i in range(num_points)]
