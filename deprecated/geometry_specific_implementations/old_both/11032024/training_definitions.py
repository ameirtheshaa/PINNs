...
def save_evaluation_results_PCA(config, device, model, model_file_path, log_folder, X_test_tensor, y_test_tensor, pca_rows, pca_columns, original_shape, epoch, epochs, use_epoch, save_name):
    mses, r2s = evaluate_model_training_PCA(config, device, model, model_file_path, X_test_tensor, y_test_tensor, pca_rows, pca_columns, original_shape)
    output_params = config["training"]["output_params_modf"]
    mse_strings = [f'MSE {param}: {mse}' for param, mse in zip(output_params, mses)]
    mse_output = ', '.join(mse_strings)
    r2_strings = [f'r2 {param}: {r2}' for param, r2 in zip(output_params, r2s)]
