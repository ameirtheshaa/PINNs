...
def load_data(config, device):
    ...
    if config["train_test"]["data_loss_test_size"] is not None:
        X_train_data, X_test_data, y_train_data, y_test_data = train_test_split(X_train, y_train, test_size=config["train_test"]["data_loss_test_size"], random_state=config["train_test"]["random_state"])
        X_train_tensor_data, y_train_tensor_data, X_test_tensor_data, y_test_tensor_data = convert_to_tensor(X_train_data, X_test_data, y_train_data, y_test_data, device=device)
    else:
        X_train_tensor_data, y_train_tensor_data, X_test_tensor_data, y_test_tensor_data = X_train_tensor, y_train_tensor, X_test_tensor, y_test_tensor
    return X_train_tensor, y_train_tensor, X_test_tensor, y_test_tensor, labels_train_tensor, X_train_tensor_skipped, y_train_tensor_skipped, X_test_tensor_skipped, y_test_tensor_skipped, X_train_tensor_data, y_train_tensor_data, X_test_tensor_data, y_test_tensor_data, feature_scaler, target_scaler

def load_data_new_angles(device, config, feature_scaler, target_scaler):
    chosen_machine_key = config["chosen_machine"]
    ...
    targets = data[config["training"]["output_params"]]
    normalized_features, normalized_targets = transform_data_with_scalers(features, targets, feature_scaler, target_scaler)
    X_train, X_test, y_train, y_test = train_test_split(normalized_features, normalized_targets,test_size=0.99999, random_state=config["train_test"]["random_state"])
