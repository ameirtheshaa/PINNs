...
def plot_div_new_angles(df,angle,config,datafolder_path,savename_total_div, savename_total_velocity,plane,cut,tolerance):
    wind_angle = angle
    if plane == 'X-Z':
        points = ['X', 'Z']
        noplotdata = 'Y'
...
def plot_div_angles_2d(df,angle,config,plot_folder):
