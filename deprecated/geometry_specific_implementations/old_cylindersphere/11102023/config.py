...
def select_device_and_batch_size(model, input_tensor):
    ...
    if len(input_tensor) < optimal_batch_size:
        optimal_batch_size = len(input_tensor)
...
def train_model(model, X_train_tensor, y_train_tensor, config, batch_size, model_file_path, epochs):
    ...
    if config["batches"]["use_batches"]:
        if not config["batches"]["force"]:
            ...
            while True:
                train_loader_temp = torch.utils.data.DataLoader(train_dataset, batch_size=optimal_batch_size, shuffle=True)
                X_batch, y_batch = next(iter(train_loader_temp))  # take a single batch
                try:
                    optimizer.zero_grad()
                    loss = closure()
                    optimizer.step()
                    optimal_batch_size += int(0.001*len(X_train_tensor))  # Increase and try again
                    print (f"current optimal batch size: {optimal_batch_size} with memory allocated: {torch.cuda.memory_allocated()/ (1024 ** 3):.2f} GB")
                except RuntimeError as e:
                    if "out of memory" in str(e):
                        optimal_batch_size -= 2*int(0.001*len(X_train_tensor))  # The last successful batch size
                        break
                    raise e 
            ...
            print (f"using batches with batch size: {optimal_batch_size} with Estimated Memory: {required_memory_optimal / (1024 ** 3):.2f} GB")
        else:
            train_dataset = torch.utils.data.TensorDataset(X_train_tensor, y_train_tensor)
            ...
            print (f"using batches with forced batch size: {batch_size} with Estimated Memory: {required_memory_forced / (1024 ** 3):.2f} GB")
    else:
    ...
    for epoch in itertools.count(start=start_epoch):  # infinite loop, will only break if the stopping condition is met.
        ...
        for X_batch, y_batch in train_loader:
            ...
            if epoch % 100 == 0:
                ...
                print(f'Epoch [{epoch}/{epochs if use_epoch else "infinity"}], Loss: {current_loss:.4f}, Total Time elapsed: {current_elapsed_time_hours:.2f} hours, with memory allocated: {torch.cuda.memory_allocated()/ (1024 ** 3):.2f} GB')
                torch.save({
                    'epoch': epoch,
                    'model_state_dict': model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
