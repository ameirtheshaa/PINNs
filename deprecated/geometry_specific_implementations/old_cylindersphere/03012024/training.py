...
def train_model(model, device, config, data_dict, model_file_path, log_folder):
    ...
    X_train_tensor, y_train_tensor, X_test_tensor, y_test_tensor, labels_train_tensor, X_train_tensor_skipped, y_train_tensor_skipped, X_test_tensor_skipped, y_test_tensor_skipped, X_train_tensor_data, y_train_tensor_data, X_test_tensor_data, y_test_tensor_data, feature_scaler, target_scaler = data_dict.values()
    ...
    if os.path.exists(model_file_path):
        ...
        recent_losses, sma, consecutive_sma_count, time_passed = load_losses_from_json(os.path.join(log_folder,'sma.json'), sma_window_size)
    ...
    for epoch in itertools.count(start=start_epoch):  # infinite loop, will only break if the stopping condition is met.
        ...
        for Xy_batch, labels in train_loader:
            ...
            def closure():
                ...
                total_loss = calculate_total_loss(X_batch, y_batch, X_batch_data, y_batch_data, epoch, config, device, model, optimizer, datafolder_path, meteo_filenames, feature_scaler, target_scaler, True)
                total_loss.backward()
                return total_loss
            ...
            if epoch % 1 == 0:
                if print_lines < 1:
                    ...
                    loss_dict = calculate_total_loss(X_batch, y_batch, X_batch_data, y_batch_data, epoch, config, device, model, optimizer, datafolder_path, meteo_filenames, feature_scaler, target_scaler, False)
