...
def main(base_directory, config, output_zip_file=None):
    ...
    if config["train_test"]["test"]:
        testing(config, device, output_folder, model_file_path, model, today, overall_start_time)
    ###TESTING###

    ###EVALUATING###
    if config["train_test"]["evaluate"]:
        evaluation(config, device, output_folder, model_file_path, model, today, overall_start_time)
    ###EVALUATING###

    ###NEW ANGLES###
    if config["train_test"]["evaluate_new_angles"]:
        evaluation_new_angles(config, device, output_folder, model_file_path, model, today, overall_start_time)
    ###NEW ANGLES###

    ##OTHERS###
    if config["train_test"]["make_logging_plots"]:
        process_logging_statistics(log_folder)
    if config["train_test"]["make_div_plots"]:
        evaluation_divergence(config, device, output_folder, model_file_path, model, today, overall_start_time)       
    if config["train_test"]["make_data_plots"]:
