...
for angle in angles:
    ...
    # cell_data_source = pvs.CellDatatoPointData(Input=data_source)

    cell_centers = pvs.CellCenters(Input=data_source)

    cell_centers_filename = os.path.join(base, "cell_centers.csv")
    ...
    combined_filename = os.path.join(base2, f"all_data_{angle}.csv")
