...
class PINN_red(nn.Module):
    def __init__(self):
        super(PINN_red, self).__init__()
    ...
    def forward(self, x):
        x = torch.relu(self.fc1(x)) #f(x) = max(0,x) tanh/swish is also a good one.
        x = torch.relu(self.fc2(x))
        x = torch.relu(self.fc3(x))
        x = torch.relu(self.fc4(x))
        stress_tensor = self.stress_tensor_layer(x)
    ...
    def extract_parameters(self, X):
        ...
        return wind_angle, x, y, z, rho, nu

    def cont(self, wind_angle, x, y, z, nu_t, rho, nu):
        # Ensure x, y, and z have requires_grad set to True
        x.requires_grad_(True)
        y.requires_grad_(True)
        z.requires_grad_(True)
        ...
        # Stack the input data
        input_data = torch.hstack((x, y, z, wind_angle, abs_cos_theta, abs_sin_theta, rho, nu))

        # Get the output and stress_tensor from the model
        output, _ = self(input_data)
