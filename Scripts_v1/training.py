...
def train_model(rank, world_size, model, device, X_train_tensor, y_train_tensor, config, batch_size, model_file_path, log_folder, epochs):
    if config["parallel_training"]["distributed_training"]:
        # Initialize distributed training environment
        device = torch.device(f'cuda:{rank}' if torch.cuda.is_available() else 'cpu')
        ip_address_master = '192.168.0.188'
        port_number = 29500
        backend = config["parallel_training"]["backend"]
    ...
    def closure():
        optimizer.zero_grad()
        if config["parallel_training"]["distributed_training"]:
            predictions, stress_tensor = model.module(X_batch)
        else:
