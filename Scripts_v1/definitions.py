...
def initialize_and_fit_scalers(features, targets):
...
def load_data(filenames, base_directory, datafolder_path, device, config):
    ...
    feature_scaler, target_scaler = initialize_and_fit_scalers(features, targets)
    ...
    features_skipped = data_skipped[['Points:0', 'Points:1', 'Points:2', 'TurbVisc', 'WindAngle', 'cos(WindAngle)', 'sin(WindAngle)', 'Density', 'Kinematic_Viscosity']]
    targets_skipped = data_skipped[['Pressure', 'Velocity:0', 'Velocity:1', 'Velocity:2']]
...
def evaluate_model(model, X_test_tensor, y_test_tensor, feature_scaler, target_scaler, output_folder):
    ...
    with torch.no_grad():
        predictions_tensor, _ = model(X_test_tensor)
        ...
        X_test_column_names = ["X", "Y", "Z", 'TurbVisc', 'WindAngle', 'cos(WindAngle)', 'sin(WindAngle)', 'Density', 'Kinematic_Viscosity']
        ...
        y_test_column_names = ['Pressure_Actual', 'Velocity_X_Actual', 'Velocity_Y_Actual', 'Velocity_Z_Actual']
        ...
        predictions_column_names = ['Pressure_Predicted', 'Velocity_X_Predicted', 'Velocity_Y_Predicted', 'Velocity_Z_Predicted']
        ...
        for wind_angle in wind_angles:
            ...
            # mask = (X_test_dataframe['WindAngle'] == wind_angle)
...
def evaluate_model_skipped(model, config, X_test_tensor, y_test_tensor, feature_scaler, target_scaler, output_folder):
    model.eval()
    test_predictions = []
    test_predictions_wind_angle = []
    with torch.no_grad():
