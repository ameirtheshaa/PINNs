...
from training_pca import *
from PINN import *
from plotting import *
from testing_pca import *
from physics import *
...
def main(base_directory, config, output_zip_file=None):
    ...
    input_params = [f"{value}_{angle}" for angle in config["training"]["input_params"] for value in config["training"]["angles_to_train"]]
    output_params = [f"{value}_{angle}" for angle in config["training"]["output_params"] for value in config["training"]["angles_to_train"]]
    ...
    if config["train_test"]["train"]:
        model = train_model_PCA(model, device, config, data_dict, model_file_path, log_folder)
    ###TRAINING###
