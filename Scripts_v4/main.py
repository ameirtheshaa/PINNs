...
def main(base_directory, config, output_zip_file=None):
    ...
    if config["train_test"]["train"]:
        ...
        if config["train_test"]["distributed_training"]:
            ...
            mp.spawn(train_model, args=(world_size, model, activation_function, device, normalized_X_train_tensor, normalized_y_train_tensor, config, feature_scaler, target_scaler, batch_size, model_file_path, log_folder, meteo_filenames, datafolder_path, epochs),nprocs=world_size,join=True)
        else:
            model = train_model(config["distributed_training"]["rank"], config["distributed_training"]["world_size"], model, device, normalized_X_train_tensor, normalized_y_train_tensor, config, feature_scaler, target_scaler, batch_size, model_file_path, log_folder, meteo_filenames, datafolder_path, epochs=config["training"]["num_epochs"])
    ###TRAINING###
