...
def train_model(model, device, config, data_dict, model_file_path, output_folder, today, overall_start_time, log_folder):
    model.train()
    ...
    if os.path.exists(model_file_path):
        ...
        if config["training"]["change_optimizer"]:
            optimizer = get_optimizer(model, chosen_optimizer_key, optimizer_config)
        else:
        ...
        for Xy_batch, labels in train_loader:
            ...
            if epoch % config["training"]["print_epochs"] == 0:
                if print_lines < 1:
                    ...
                    free_memory, _ = get_available_device_memory(config, device.index)
                    ...
                    save_model(model, model_file_path, epoch, current_loss, training_completed, optimizer, start_time)
                    print_lines += 1
            if epoch % config["training"]["save_metrics"] == 0:
...
def train_model_PCA(model, device, config, data_dict, model_file_path, output_folder, today, overall_start_time, log_folder):
    model.train()
    ...
    for epoch in itertools.count(start=start_epoch):  # infinite loop, will only break if the stopping condition is met.
        ...
        if epoch % config["training"]["print_epochs"] == 0:
            if print_lines < 1:
                ...
                current_elapsed_time_hours = current_elapsed_time / 3600
                free_memory, _ = get_available_device_memory(config, device.index)
                loss_dict = calculate_total_loss(X_batch, y_batch, epoch, config, model, optimizer, False, no_slip_features_normals, all_inlet_features_targets, y_batch_divergences)
                print_statement(epoch, epochs, use_epoch, current_loss, loss_dict, free_memory, current_elapsed_time_hours)
                success = False
                error_count = 0
                while not success:
                ...
                print_lines += 1
        if epoch % config["training"]["save_metrics"] == 0:
            save_evaluation_results_PCA(config, device, model, model_file_path, log_folder, X_train_tensor, y_train_tensor, epoch, epochs, use_epoch, x_training, save_name='info_test.csv')
            save_evaluation_results_PCA(config, device, model, model_file_path, log_folder, X_test_tensor_skipped, y_test_tensor_skipped, epoch, epochs, use_epoch, x_skipped, save_name='info_skipped.csv')
...
def train_model_fft(model, device, config, data_dict, model_file_path, output_folder, today, overall_start_time, log_folder):
    model.train()
    ...
    for epoch in itertools.count(start=start_epoch):  # infinite loop, will only break if the stopping condition is met.
        ...
        if epoch % config["training"]["print_epochs"] == 0:
            if print_lines < 1:
                ...
                print_lines += 1
        if epoch % config["training"]["save_metrics"] == 0:
            save_evaluation_results_fft(config, device, model, model_file_path, log_folder, X_train_tensor, y_train_tensor, epoch, epochs, use_epoch, x_training, save_name='info_test.csv')
            save_evaluation_results_fft(config, device, model, model_file_path, log_folder, X_test_tensor_skipped, y_test_tensor_skipped, epoch, epochs, use_epoch, x_skipped, save_name='info_skipped.csv')
