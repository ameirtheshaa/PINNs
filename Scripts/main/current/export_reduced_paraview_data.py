...
for angle in angles:
    ...
    extracted_surface = pvs.ExtractSurface(Input=data_source)

    cell_centers = pvs.CellCenters(Input=extracted_surface)

    cell_centers_filename = os.path.join(base2, f"Paraview_Reduced_CFD_cell_data_simulation_{angle}.csv")
    pvs.SaveData(cell_centers_filename, proxy=cell_centers)

    pvs.UpdatePipeline()

    # # First-order gradients
    # velocity_gradient = pvs.Gradient(Input=extracted_surface, ScalarArray=['CELLS', 'Velocity'], ResultArrayName='GradientVelocity')
    # pvs.UpdatePipeline()
    # pressure_gradient = pvs.Gradient(Input=extracted_surface, ScalarArray=['CELLS', 'Pressure'], ResultArrayName='GradientPressure')
    # pvs.UpdatePipeline()
    # turbvisc_gradient = pvs.Gradient(Input=extracted_surface, ScalarArray=['CELLS', 'TurbVisc'], ResultArrayName='GradientTurbVisc')
    # pvs.UpdatePipeline()
    ...
    # extracted_surface_vtk = pvs.servermanager.Fetch(extracted_surface)
    ...
    # print("Number of points in extracted_surface:", extracted_surface_vtk.GetNumberOfPoints())
    # print("Number of cells in extracted_surface:", extracted_surface_vtk.GetNumberOfCells())
