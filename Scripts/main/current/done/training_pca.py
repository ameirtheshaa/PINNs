...
def train_model_PCA(model, device, config, data_dict, model_file_path, log_folder):
    model.train()
    ...
    for epoch in itertools.count(start=start_epoch):  # infinite loop, will only break if the stopping condition is met.
        ...
        if epoch % config["training"]["print_epochs"] == 0:
            if print_lines < 1:
                ...
                save_model(model, model_file_path, epoch, current_loss, training_completed, optimizer, start_time)
                save_evaluation_results_PCA(config, device, model, model_file_path, log_folder, X_train_tensor, y_train_tensor, pca_rows, pca_columns, original_shape, epoch, epochs, use_epoch, save_name='info_test.csv')
                save_evaluation_results_PCA(config, device, model, model_file_path, log_folder, X_test_tensor_skipped, y_test_tensor_skipped, pca_rows, pca_columns, original_shape, epoch, epochs, use_epoch, save_name='info_skipped.csv')
                print_lines += 1
