...
def adaptive_loss(config, data_loss, inlet_loss, no_slip_loss, cont_loss, momentum_loss, epoch, max_epochs):
...
def gradient_magnitude_loss(data_loss, inlet_loss, no_slip_loss, cont_loss, momentum_loss, model, optimizer):
    """
...
def weighting(config, data_loss, inlet_loss, no_slip_loss, cont_loss, momentum_loss, epoch, max_epochs, model, optimizer):

    if config['loss_components']['use_weighting']:
        if config['loss_components']['weighting_scheme'] == 'adaptive_weighting':
            return adaptive_loss(config, data_loss, inlet_loss, no_slip_loss, cont_loss, momentum_loss, epoch, max_epochs)
        elif config['loss_components']['weighting_scheme'] == 'gradient_magnitude':
            return gradient_magnitude_loss(data_loss, inlet_loss, no_slip_loss, cont_loss, momentum_loss, model, optimizer=optimizer)
        else:
            raise ValueError("Unknown weighting scheme")
    else:
        # Sum up all active loss components when no weighting is applied
        return sum(active_losses)